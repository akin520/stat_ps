...
if($quick -gt 0){
    $sqltmp+="(1,1,1,1,1,0,'$Date',$hours,1,$quick)"
}
$activ=$dt1.Tables[1].rows[0][0]
if($activ -gt 0){
    $sqltmp+="(1,1,1,1,1,0,'$Date',$hours,2,$activ)"
}
$count=$dt1.Tables[2].rows[0][0]
if($count -gt 0){
    $sqltmp+="(1,1,1,1,1,0,'$Date',$hours,3,$count)"
}
...
$cc.CommandText="INSERT INTO [KysdStatisticsDB].[dbo].[Register1Hour]([GamesID],[OSID],[OSVersionID],[ChannelID],[SpreadChannelID],[SpreadSubChannelID],[Date],[Hour],[RegisterTypeID],[Count]) VALUES $sqlmore"
